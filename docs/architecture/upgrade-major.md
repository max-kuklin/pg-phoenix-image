...
upgrade (entrypoint detects version mismatch + PG_UPGRADE=true)
  ...
  │   ├─ run `/tmp/pg_upgrade/analyze_new_cluster.sh` if `pg_upgrade` generated it
  │   ├─ otherwise run `vacuumdb --all --analyze-in-stages --missing-stats-only`
  │   │   followed by `vacuumdb --all --analyze-only`
...
`tests/upgrade.test.js` spins up pg-phoenix-image with two PostgreSQL major versions plus SeaweedFS S3 for the real upgrade path. `tests/contracts/upgrade-script.test.js` covers deterministic rollback branches with fake PostgreSQL binaries so failure can be injected exactly before and after the PGDATA swap.

Covered now:

- Version mismatch without the gate refuses startup with a clear error
- Missing old binary stash fails closed before upgrade starts
- Pre-upgrade backup gate starts old stashed PostgreSQL on localhost and pushes a backup when needed
- Real 17-to-18 upgrade completes, keeps data intact, runs post-upgrade analyze, switches backup prefix, and removes the old binary stash
- `pg_upgrade --link` failure leaves old PGDATA in place and removes `$PGDATA.new`
- Post-upgrade temporary start failure swaps `$PGDATA.old` back to active PGDATA
- Restart with `PG_UPGRADE=true` still set starts normally once PGDATA already matches the image major
- Same-major binary changes refresh the checksum-based stash

Remaining checks depend on a real deployment shape rather than the image itself: Kubernetes probe timing, old-prefix cleanup timing, and application-specific smoke checks after PostgreSQL starts.
