...
> Note: The Dockerfile is committed in its final state. It references image runtime scripts from all phases. Phase 1 creates the test harness, logger, and minimal scripts so the image can build and container-first TDD can start immediately. Later phases replace stubs with real implementations.
...
| 2 | `image/lib/logger.sh`, `tests/contracts/logger.test.js` | step 1 | contract test runs in Linux |
| 3 | stub scripts: `image/entrypoint.sh`, `image/backup.sh`, `image/restore.sh`, `image/upgrade.sh` | steps 1-2 | scripts are executable and source logger cleanly |
...
| 7 | `image/backup.sh`, `image/restore.sh`, and WAL-G startup setup in `entrypoint.sh` | Phase 2 plus step 6 | image still starts; contract tests pass |
...
| Step | Deliverable | Depends on | Test |
|---|---|---|---|
| 9a | startup restore selection in `image/entrypoint.sh` | Phase 1 plus steps 6-7 | startup refusal and restore-path scenarios |
...
| Step | Deliverable | Depends on | Test |
|---|---|---|---|
| 12a | `image/upgrade.sh` preflight checks for env, old binary stash, and basic gates | Phase 1 plus steps 6 and 9 | contract tests only |
| 12b | `image/upgrade.sh` backup gate and old-version temporary startup | step 12a | contract tests for decisions, container tests for process behavior |
...
|---|---|---|---|
| 1: Foundation | `logger.sh`, image runtime script stubs | contract harness, logger contract, container helpers, image build gate | 10 files |
